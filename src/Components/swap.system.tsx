...
export default class SwapSystem extends React.Component<
  IPSWidgetProps,
  IPSWidgetState
> {
  paraswap: ParaSwap;
  ...
  constructor(props: IPSWidgetProps) {
    ...
    this.onAmountChangeEvt = {
      srcAmount: _.debounce((value: any) => this.setSrcAmount(value), 500),
    };
  ...
  async getBestPrice(srcAmount: string) {
    try {
      ...
      if (
        new BN(srcAmount).isNaN() ||
        new BN(srcAmount).isLessThanOrEqualTo(0) ||
        !tokenFrom ||
        !tokenTo
      ) {
        return;
      ...
      const priceRouteOrError: any = await this.paraswap.getRate(
        tokenFrom.address,
        tokenTo.address,
        _srcAmount.toFixed(0)
      );

      if (priceRouteOrError.message) {
        return this.setState({
          error: priceRouteOrError.message,
          loading: false,
        });
      }
      ...
      this.setState(
        {
          ...
          tokenTo: this.state.tokens.find((t: any) => t.symbol === symbol),
        },
        () => this.getBestPrice(this.state.srcAmount)
      );
    }
  ...
  getDestAmount() {
    ...
    const destAmount = new BN(priceRoute.amount).dividedBy(
      10 ** tokenTo.decimals
    );
  ...
  setSrcAmount(value: string) {
    ...
    this.setState({ srcAmount, priceRoute: undefined }, () =>
      this.getBestPrice(srcAmount)
    );
  }
  ...
  async connectUser(): Promise<User | APIError> {
    try {
      ...
      const user = new User(
        addresses[0],
        Number(web3.networkVersion) as NetworkID,
        userTokens as Token[]
      );
    ...
    return new Promise(async (resolve, reject) => {
      return setTimeout(async () => {
        try {
          ...
          const allowance = await this.paraswap.getAllowance(
            user!.address,
            tokenFrom!.address
          );

          if (
            new BN(allowance).isGreaterThanOrEqualTo(
              new BN(srcAmount).times(10 ** tokenFrom!.decimals)
            )
          ) {
  ...
  async checkBalances() {
    try {
      ...
      const enoughBalance = (balance: any, amount: any) =>
        new BN(balance).isGreaterThanOrEqualTo(amount);
      const enoughAllowance = (allowance: any, amount: any) =>
        new BN(allowance).isGreaterThanOrEqualTo(amount);

      const { balance, allowance } =
        (user!.tokens || []).find((t: any) => t.symbol === tokenFrom!.symbol) ||
        {};

      const _srcAmount = new BN(srcAmount)
      ...
      if (tokenFrom!.symbol !== "ETH") {
        if (!enoughAllowance(allowance, _srcAmount)) {
          ...
          const allowance = unlimitedAllowance
            ? UNLIMITED_ALLOWANCE
            : _srcAmount;

          const transactionHash = await this.paraswap.approveToken(
            allowance,
            user!.address,
            tokenFrom!.address
          );
  ...
  async swap() {
    try {
      ...
      const { tokenFrom, tokenTo, srcAmount, priceRoute, receiver } =
        this.state;
      ...
      if ((userOrError as APIError).message) {
        return this.setState({
          error: (userOrError as APIError).message,
          loading: false,
        });
      }
      ...
      const minDestinationAmount = new BN(priceRoute.amount)
        .multipliedBy(1 - defaultSlippage)
        .toFixed(0);
      ...
      await this.provider.eth.sendTransaction(
        txParams,
  ...
  isOutOfBalance() {
    ...
    const token = (user.tokens || []).find(
      (t: any) => t.symbol === tokenFrom!.symbol
    );

    return (
      !!token &&
      new BN(token.balance || "0")
        .dividedBy(10 ** tokenFrom!.decimals)
        .isLessThan(srcAmount)
    );
  }
  ...
  getBalance(fromOrTo: any) {
    ...
    const token =
      selectedToken &&
      user.tokens.find((t: any) => t.symbol === selectedToken.symbol);

    const balance = token
      ? token.balance &&
        new BN(token.balance).dividedBy(10 ** token.decimals).toFixed(4)
      : 0;

    const outOfBalance = fromOrTo === "from" ? this.isOutOfBalance() : false;

    return (
      <Label className={`balance outOfBalance-${outOfBalance}`}>
        {balance}
      </Label>
    );
  }
  ...
  render() {
    ...
    const { bgColor, hasReceiver, defaultPair, fixedFrom, fixedTo } =
      this.props;
        ...
        <Form>
          <Form.Field>
            <Input
              ...
              placeholder="Send"
              onChange={(e: any) =>
                this.onAmountChange("srcAmount", e.target.value)
              }
